...
export default function DashboardPage() {
  ...
  const [chat, setChat] = useState<ChatMsg[]>([
    { role: "assistant", content: "Hi! Upload a receipt or tell me an expense. You can also ask: Can I afford a $2000 trip?" }
  ]);

  // Simple user finance settings (local-only for hackathon).
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setUserId(id);
      if (!id) setStatus("Not logged in (you can still browse). Go to /login to sign in.");
    })();
  }, []);
  ...
  async function uploadReceipt(file: File) {
    ...
    const res = await fetch("/api/extract-receipt", {
      ...
      body: JSON.stringify({ user_id: userId, receipt_url: receiptUrl })
    });
  ...
  return (
    <div className="stack">
      <div className="card">
        ...
        <div className="muted">
          User: {userId ?? "Not logged in"} • Last 30d spend: <b>${last30Spend.toFixed(2)}</b>
        </div>
        ...
        <div style={{marginTop:12}}>
          <label className="muted">Upload receipt photo (jpg/png)</label><br/>
          <input
            type="file"
            accept="image/*"
            onChange={(e) => {
      ...
      <div className="row">
        <div className="card" style={{flex: 2, minWidth: 360}}>
          ...
          <div style={{height: 260, overflow:"auto", border:"1px solid #eee", borderRadius: 12, padding: 10}}>
            {chat.map((m, i) => (
      ...
      <div className="card">
        ...
        <ul className="muted" style={{marginBottom:0}}>
          <li>Add a pie chart endpoint (server) + chart component (client) for category breakdown.</li>
          <li>Implement recurring charge detection in <code>app/api/chat/route.ts</code> using merchant + monthly pattern.</li>
          <li>Implement screenshot OCR or use OpenAI vision on bank statement screenshots.</li>
          <li>Add city cost-of-living data and answer “Which city should I live in…” more concretely.</li>
        </ul>
      </div>
    </div>
  );
